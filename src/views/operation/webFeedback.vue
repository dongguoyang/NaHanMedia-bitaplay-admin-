          ...
          >
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <el-button slot="reference" type="primary" size="mini" v-else @click="handelAction(scope.row.id)">处理
                </el-button>
              </template>
            </el-table-column>
...
<script>
import {getWebFeedbackList, handelWebFeedback} from "@/api/webFeedback";
...
export default {
  ...
  methods: {
    getList() {
      ...
      getWebFeedbackList(this.table.query).then(({code, data}) => {
    ...
    handelAction(id) {
      ...
      this.$prompt('请输入备注', '备注', {
        ...
        handelWebFeedback(this.handel).then(({code}) => {
          if (code == 0) {
