      ...
      <el-main>
        <el-card class="box-card">
          ...
          >
            ...
            <el-table-column label="应得金额">
              <template slot-scope="scope">
                ￥{{ scope.row.reward_amount / 100 }}
              </template>
            </el-table-column>

            <el-table-column label="实得金额">
              <template slot-scope="scope">
                ￥{{ scope.row.user_reward_amount / 100 }}
              </template>
            </el-table-column>


            <el-table-column prop="created_at" label="奖励时间" min-width="130"></el-table-column>
          </el-table>
...
<script>
import {getUserDownloadReward} from "@/api/userFinance";
...
export default {
  ...
  methods: {
    getList() {
      ...
      getUserDownloadReward(this.table.query).then(({code, data}) => {
