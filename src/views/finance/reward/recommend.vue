      ...
      <el-main>
        <el-card class="box-card">
          ...
          >
            ...
            <el-table-column label="服务商ID">
              <template slot-scope="scope">
                {{ scope.row.provider.id }}
              </template>
            </el-table-column>
            <el-table-column label="服务商手机号">
              <template slot-scope="scope">
...
<script>
import {getUserRecommendReward} from "@/api/userFinance";
...
export default {
  ...
  methods: {
    getList() {
      ...
      getUserRecommendReward(this.table.query).then(({code, data}) => {
